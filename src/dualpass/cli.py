...
Top-level commands:
  ...
  doctor       — probe the environment (CLIs, Python, state directory)
...
v0.1.0a0 status:
  Only `--version` and `--help` are functional. Every other command emits a structured
  NotImplementedError pointing at the milestone where it lands. See CHANGELOG.md.
"""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser(
        "doctor",
        help="Probe the environment for agent CLIs and required services",
        description=(
            "Non-destructive health check. Reports Python version, uv version, "
            "which agent CLIs are installed, state-directory writability, and any "
            "stale lockfiles in the current working directory."
        ),
    )
...
_MILESTONE = "v0.2.0 (controller, config loader, mock provider)"
_WATCHER_MILESTONE = "v0.3.0 (watchers + state seeding)"


def _stub(command: str, milestone: str = _MILESTONE) -> int:
    """Print a structured 'not yet implemented' message and exit 2."""
    msg = (
        f"dualpass: '{command}' is not yet implemented.\n"
        f"  v0.1.0a0 ships scaffolding only. This command lands in {milestone}.\n"

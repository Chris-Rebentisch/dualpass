...
description: Judge the outline-author's artifact via the compute-floor + live-fire-probe contract.
context_sources:
  - units/{unit_id}/outline-v{round}.md
artifacts_produced:
  - units/{unit_id}/outline-v{round}-review.md
success_criteria:
...
# outline-reviewer skill (template)
...
Write `units/{unit_id}/outline-v{round}-review.md` with:
